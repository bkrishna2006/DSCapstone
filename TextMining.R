...
freqLimit <- c(0,2000) # this range has been set after a few iterations to determine which one is most useful
...
setwd(myGenDataDir)
neededObjects <- c("removingEngStopwordsTime",
                   "removingProfWordsTime",
                   "myNewCorpus",
                   "myNewWordCloud",
                   "myBarPlot",
                   "myScriptsDir")

for (i in 1:length(neededObjects)){
#  saveRDS(neededObjects,paste0(myGenDataDir,neededObjects[i],".RData"))
  saveRDS(neededObjects,paste0("./",neededObjects[i],".RData"))
}
                
rm(list=setdiff(ls(),neededObjects))
gc()

# This program is intended primarily to create a clean corpus.
